...
# scatter charts with points (circles or other shapes)

use v5.12;

package Chart::Scatter;
our @ISA     = qw(Chart::Base);
our $VERSION = 'v2.403.9'; # based on Chart::Base
...
sub _draw_data
{
    ...
    my $x_range = abs($self->{'x_max_val'} - $self->{'x_min_val'});
    ...
    my $x_min_value = $data->[0][0];
    my $x_max_value = $data->[0][0];
    foreach my $j (0 .. $self->{'num_datapoints'} ) {
       if ($data->[0][$j] < $x_min_value) {
          $x_min_value = $data->[0][$j];
       }
       if ($data->[0][$j] > $x_max_value) {
          $x_max_value = $data->[0][$j];
       }
    }
    my $x_range = $x_max_value - $x_min_value;
    # draw the points
    for $i ( 1 .. $self->{'num_datasets'} )
    {
        ...
        for $j ( 0 .. $self->{'num_datapoints'} )
        {
            ...
            if ( defined( $data->[$i][$j] ) )
            {
                ...
                {
                    $x2 = $x1 + ($width * ( ( $data->[0][$j] - $x_min_value ) / $x_range) );
                    $x3 = $x2;
                }
